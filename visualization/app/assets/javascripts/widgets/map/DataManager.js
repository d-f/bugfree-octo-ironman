/** This data manager class listeners for new posts and creates the map markers and category entries.
 * @param receiver Middleware object, we receive posts from it and send category updates to it. */
function DataManager (receiver) {
  ...
  // Subscription for the interval change event. 
  receiver.subscribe_IntervalChange (function (maximum) {
    for (var i = 0; i < data.length; i ++) {
      
      //TODO Hey, this shouldn't happen here. The dispatcher is supposed to send us valid data!
      var date = data[i]['date'];   
      var reference = new Date ();
      var maxOld = reference.setMinutes (reference.getMinutes() - maximum); 
      //if ();
      
      //TODO Scheiße, das bringt nix !!
      // Das Datum ist das echte, nicht die simulierte Zeit!
      
      // Referenzformat in dem Array: "2013-10-29T13:17:59.000Z"
    }
  });
  ...
  function newPost (post) {
    ...
    if (post['geolocation'] != null) {
      ...
      // Also format the position. The user probably doesn't want eight decimal places ...
      var accuracy = 4;
