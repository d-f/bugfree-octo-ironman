...
public class DBAdapterImpl {
    ...
    //TODO Abfrage bzw. Tabelle �bberarbeiten einige Orte sind nicht in der Tabelle, obwohl in der Datenbank! Beispiel Wittenberg und Zug!
    ...
    private void getAlleStaedteNamen() { //TODO Abfrage bzw. Tabelle �bberarbeiten einige Orte sind nicht in der Tabelle, obwohl in der Datenbank! Beispiel Wittenberg und Zug!
        open();
    ...
    //Hinzugef�gt (AWH) zum Testen, da andere Methoden nicht brauchbar
    ...
    public SocialMessage[] getSocialMessagesArray(String table, Timestamp begin, Timestamp end) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				sm.setPlace(resultSet.getString(5));//Hinzugef�gt um Ort zu erhalten (AWH)
				messages.add(sm);
			}
